...
public class FomBottomTabsView extends FrameLayout implements ViewPager.OnPageChangeListener {
    ...
    public FomBottomTabsView(@NonNull Context context) {
        this(context, null);
    }

    public FomBottomTabsView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public FomBottomTabsView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
   ...
   public void setUpWithViewPager(ViewPager viewPager){
    ...
    private void init(){
        LayoutInflater.from(getContext()).inflate(R.layout.view_fom_bottom_tabs, this, true);
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        if(position == 0){
            ...
            mStartImage.setColorFilter(ContextCompat.getColor(getContext(), R.color.white), android.graphics.PorterDuff.Mode.SRC_IN);
        }else if(position == 1){
            ...
            moveAndScaleCenter(1 - positionOffset);
            mEndImage.setColorFilter(ContextCompat.getColor(getContext(), R.color.white), android.graphics.PorterDuff.Mode.SRC_IN);
        }
    }
